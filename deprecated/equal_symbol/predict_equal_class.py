...
class CustomDataset(Dataset):
    def __init__(self, root, transform=None):
        ...
        self.image_paths = [os.path.join(root, filename) for filename in os.listdir(root) if
...
# 加载测试数据集
test_dataset = CustomDataset(root="../../workdir/Spilt/ori_gray_div_last", transform=transform)
...
# 加载已训练的模型参数
model.load_state_dict(torch.load("../../workdir/Models/resnet18_equal.pth", map_location=device))

# 设置模型为评估模式
model.eval()

# 创建目标目录
output_dir = '../../workdir/Test/Predictions_Classify_Equal'
